$(function () {
    ...
    $('.prev-month, .next-month, #current-month').on('click', function () {
        ...
        $.ajax({
            type: "POST",
            url: "ajax/family-summary-change-day.php",
            data: {
                'action_val': value,
...
function checkMonthYearMatch(first_unix_timestamp, second_unix_timestamp) {
    ...
    const targetDate = stringFirstDate.getDate();
    ...
    const currentDate = stringSecondDate.getDate();

    if (targetMonth == currentMonth && targetYear == currentYear && targetDate == currentDate) {
        return true;
    } else {
        return false;
    }
